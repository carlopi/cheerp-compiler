						  ...
						  SmallVector<Value*, 4>& initialArgs)
{
	...
	//call entirely
	assert(isBuiltinConstructorForType(cast<CallBase>(ci)->getCalledFunction()->getName().data(), "class._ZN6client6StringE") == redundantStringConstructor(callInst, initialArgs));
